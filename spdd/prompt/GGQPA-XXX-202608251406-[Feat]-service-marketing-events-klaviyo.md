...
20. **Catalog orphan variants (implementation):** After upserting current Lunar variants, list remote variants for the catalog item and delete orphans whose `external_id` is not a current variant SKU — cleans up legacy DB-id keyed variants when identity is SKU-based.
21. **CatalogExternalIdStore (implementation):** Persist item `external_id` by product id (`Cache::forever`) on sync / product deleting / variant delete capture; **DELETE** reads the store; forget after successful product DELETE. **Not** sufficient alone for upsert create-vs-update routing — use Klaviyo remote GET for that.
...
24. **Catalog wipe command (implementation):** `klaviyo:delete-all-products {--force}` dispatches `DeleteAllProductsFromKlaviyo` on the application default queue; job lists remote catalog items and submits Klaviyo bulk-delete jobs (variants removed with parent items). Requires `enabled` only (not `sync_products`).
...
27. **Deferred vs default queue (split):** Profiles, subscribe, storefront events, orders, and **single-product** catalog upsert/delete from product/variant/admin lifecycle dispatch with `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` — process after the HTTP response (mirror lunar-frontend GTM; no helper dispatcher class). Batch backfill (`SyncAllProductsToKlaviyo` bulk chunks), discount-driven catalog re-syncs (`ResolvesDiscountables` → batched bulk sync / `dispatchFullCatalogSync`), and catalog wipe (`DeleteAllProductsFromKlaviyo`) stay on the **application default queue** (bare `dispatch()`, no `onConnection`). Package ensures `queue.connections.deferred` exists when missing via `KlaviyoServiceProvider::registerDeferredQueueConnection()` and registers `InvokeDeferredCallbacks` HTTP middleware via `registerDeferredQueueMiddleware()` so deferred jobs actually run on hosts with custom/pre-Laravel-12 HTTP kernels (without this middleware, listeners may log “dispatching SyncProductToKlaviyo” but `handle()` never runs). Ops may override deferred connection via `KLAVIYO_QUEUE_CONNECTION`.
28. **Catalog upsert transport split (implemented — M14 + M15):** **Batch backfill and discount re-sync** (`syncProductsBulk` default) submit Klaviyo **async bulk catalog jobs** only — no synchronous single-item create/update HTTP. **Create:** `POST /api/catalog-item-bulk-create-jobs` and `POST /api/catalog-variant-bulk-create-jobs`. **Update (existing remote resources only):** `POST /api/catalog-item-bulk-update-jobs` and `POST /api/catalog-variant-bulk-update-jobs`. **Single-product lifecycle/admin sync** (`syncProduct` → `syncProductsBulk(..., synchronousUpdates: true)`) uses synchronous **PATCH** for existing remote resources: `UpdateCatalogItemRequest` (single item) and `UpdateCatalogVariantRequest` (each variant); **creates** still use bulk jobs with item-create polling before variant create. **Bulk update does not insert** missing items or variants. Service chooses create vs update per **item and per variant** using Klaviyo remote state (`GET /catalog-items/{compoundId}/relationships/variants/` — 404 → create item; 200 + empty → update item + create all variants; 200 + ids → update item + create/update each variant by presence). `CatalogExternalIdStore` alone is **not** sufficient for upsert routing (DELETE capture only). After item bulk create jobs are accepted, **poll** `GET /catalog-item-bulk-create-jobs/{job_id}` until `status=complete` (throw on `failed_count > 0`) **before** variant bulk create — parent item must exist. Variant bulk **update** payloads must **omit** `relationships` (Klaviyo rejects `item` relation on update; create only). Each bulk request accepts ≤100 resources; chunk accordingly. Items bulk jobs first, then variants. **Forbidden in batch upsert hot path:** `UpdateCatalogItemRequest` / `UpdateCatalogVariantRequest`. **Required in lifecycle single-product update path:** synchronous PATCH so admin saves converge item + variant title/price immediately (async bulk variant update alone left stale variant titles). Deletes, orphan cleanup, and wipe stay on existing single/bulk-delete paths; orphan list treats item 404 as no-op.
29. **Catalog bulk upsert routing hardening (implementation):** Regression fixes from backfill testing: never route all variants to bulk update when item exists but variants never synced; never throw on orphan cleanup when remote item absent (404); distinguish item missing (404) vs item exists with zero variants (200 empty) when choosing create/update paths.
30. **Lifecycle sync reliability (implementation — M15):** `SyncProductToKlaviyo` sets `uniqueFor = 7200` so an interrupted deferred dispatch cannot hold the `ShouldBeUnique` lock forever (symptom: listener logs dispatch but subsequent admin saves never sync). Host application **terminating callbacks** must not throw uncaught exceptions after the HTTP response — otherwise deferred jobs queued during request termination may never run (e.g. Lunar `TelemetryService` posting to stats must swallow 429/network failures; core package makes telemetry best-effort). Pest must assert `InvokeDeferredCallbacks` middleware is registered by `KlaviyoServiceProvider` and lifecycle sync sends `UpdateCatalogVariantRequest` (not only bulk variant update) when updating existing variants.
...
- Profiles, subscribe, events, orders, and single-product catalog upsert/delete from product/variant/admin lifecycle must dispatch with `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` (base value `deferred`), matching lunar-frontend GTM — no helper dispatcher class. Package must register `InvokeDeferredCallbacks` middleware so deferred jobs run on custom HTTP kernels. Batch catalog backfill (`SyncAllProductsToKlaviyo` bulk chunks), discount-driven re-syncs (`ResolvesDiscountables`), and catalog wipe (`DeleteAllProductsFromKlaviyo`) must use the **application default queue** (bare `dispatch()`, no `onConnection`). Batch upserts use Klaviyo bulk create/update jobs only; lifecycle single-product updates use synchronous PATCH for existing item + variants.
...
class DeleteAllProductsFromKlaviyo {
    +int pageSize
    +handle() void
}
...
class KlaviyoCatalogService {
    +syncProduct(product) array
    +syncProductsBulk(products, synchronousUpdates) array
    +updateCatalogItem(itemCompoundId, payload) array
    +updateCatalogVariant(variantCompoundId, payload) array
    +submitCatalogItemBulkCreate(items) array
    +submitCatalogItemBulkUpdate(items) array
    +submitCatalogVariantBulkCreate(variants) array
    +submitCatalogVariantBulkUpdate(variants) array
    +waitForBulkCatalogItemCreateJobs(jobResponses) void
    +waitForBulkCatalogItemCreateJob(jobId) void
    ...
    +buildCatalogItemPayload(product) array
    +buildCatalogVariantPayloads(product) array
...
SyncAllProductsToKlaviyo --> KlaviyoCatalogService : syncProductsBulk
DeleteAllProductsFromKlaviyoCommand --> DeleteAllProductsFromKlaviyo
DeleteAllProductsFromKlaviyo --> KlaviyoCatalogService : deleteAllCatalogItems
...
2. Technical implementation:
   ...
   - **Klaviyo queue connection:**
     - **Deferred:** profile upsert, subscribe, storefront track event, order sync, and **single-product** catalog jobs from product/variant/admin lifecycle listeners (`SyncProductToKlaviyo`, `DeleteCatalogVariantFromKlaviyo`, and that job’s nested parent UPDATE/DELETE). Use `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` inline (same as GTMEventHandler — **no** `KlaviyoDispatcher` helper). Env `KLAVIYO_QUEUE_CONNECTION` overrides this connection only.
     - **Application default queue:** `SyncAllProductsToKlaviyo` (Artisan `klaviyo:sync-all-products`) submits bulk catalog job chunks via `syncProductsBulk`; discount-driven paths via `ResolvesDiscountables` (`dispatchProductSyncs` batched bulk / `dispatchFullCatalogSync`); catalog wipe via `DeleteAllProductsFromKlaviyo` job. Bare `dispatch()` — do **not** call `onConnection('deferred')` (batch work must not run in-process after the HTTP response).
     - **Catalog wipe:** `klaviyo:delete-all-products` dispatches `DeleteAllProductsFromKlaviyo` on the application default queue; job calls `deleteAllCatalogItems()` (paginated list + Klaviyo bulk-delete jobs). Not on deferred connection.
     - `KlaviyoServiceProvider::register()` calls `registerDeferredQueueConnection()` so `queue.connections.deferred` exists when the host’s `queue.php` predates the deferred driver, and `registerDeferredQueueMiddleware()` pushes `InvokeDeferredCallbacks` onto the HTTP kernel when absent — required for deferred jobs to run after the response on custom kernels. Mailchimp unchanged.
...
3. Business logic:
   ...
   - **Behavioral event ↔ catalog identity:** Neutral storefront properties keep Lunar DB ids (`product_id`, `product_id_{n}`, `variant_id`, optional `variant_id_{n}`) for Mailchimp. `TrackEventToKlaviyo` must call `KlaviyoCatalogService::mapEventProductIdentifiers` before Create Event so Klaviyo receives SKUs: `product_id*` → same algorithm as catalog item `external_id` / `resolveItemExternalId` (first non-empty variant SKU, then store; never raw product DB id when a SKU exists); `variant_id*` → that variant’s SKU (`sku` / `sku_{n}` when present, else variant lookup). **Forbidden:** shipping raw Lunar product/variant DB ids on Klaviyo behavioral metrics when a SKU exists. Order metrics: case-sensitive `ProductID` / `VariantID` = catalog item / variant `external_id` (both SKU-based via `resolveItemExternalId` / `resolveVariantExternalId`).
   ...
   - **Klaviyo catalog (provider-owned, not a shared marketing event):** Marketing uses Klaviyo Catalogs API so email product-recommendation blocks can resolve the full catalog. Parity target is Mailchimp’s outcome (products present remotely), not Mailchimp’s ecommerce PUT shape.
     ...
     - **Orphan cleanup:** after upserting current variants, `deleteOrphanCatalogVariants` lists remote variants for the item and deletes those whose `external_id` is not a current variant SKU (cleans up legacy DB-id keyed remote variants). Item **404** during list → skip silently (no throw).
     ...
     - Backfill path: Artisan `klaviyo:sync-all-products` dispatches `SyncAllProductsToKlaviyo`, which chunks available products (≤100 per chunk) and submits **bulk catalog item + variant jobs** per chunk — not one Laravel job per product calling single-item HTTP.
     - Wipe path: Artisan `klaviyo:delete-all-products {--force}` dispatches `DeleteAllProductsFromKlaviyo` (paginated list + bulk-delete jobs via `deleteAllCatalogItems()`); requires `enabled` only; variants deleted with parent items.
     ...
     - Catalog HTTP stays in Saloon requests under `packages/klaviyo`; service maps Lunar `Product` + variants → bulk job payloads (+ ensure categories via single create). **Bulk upsert requests (batch backfill/discount):** `BulkCreateCatalogItemsRequest`, `BulkCreateCatalogVariantsRequest`, `BulkUpdateCatalogItemsRequest`, `BulkUpdateCatalogVariantsRequest`. **Lifecycle single-product update requests (synchronous PATCH when resource exists remotely):** `UpdateCatalogItemRequest`, `UpdateCatalogVariantRequest` — used only from `syncProduct` / `syncProductsBulk(..., synchronousUpdates: true)`; item PATCH when exactly one item in update batch; each existing variant PATCHed individually. **Job status (required for item create):** `GetBulkCreateCatalogItemsJobRequest` — poll until `complete` before variant bulk create; throw on `failed_count > 0`. **Delete/list (unchanged):** `GetCatalogItemsRequest`, `GetCatalogItemVariantIdsRequest`, `DeleteCatalogVariantRequest`, `BulkDeleteCatalogItemsRequest`, `DeleteCatalogItemRequest`. **Forbidden in batch upsert hot path:** `UpdateCatalogItemRequest`, `UpdateCatalogVariantRequest`. **Forbidden in lifecycle update path:** relying on async bulk variant update alone when admin expects immediate item + variant title convergence.
     - **Order line identity:** `KlaviyoOrderService` must set each line’s case-sensitive `ProductID` to `resolveItemExternalId(product)` and `VariantID` to `resolveVariantExternalId(variant)` when the purchasable is a variant — never `ProductId` / `VariantId` casing or raw Lunar DB ids when a SKU exists. Emit a stable-id `Ordered Product` event per line in addition to aggregate `Placed Order` when product-level purchase behavior is required.
...
2. Provider listeners: thin classes; prefer `ShouldQueue` only if translation is expensive — default **sync listener → async job**. Profiles/events/orders/single-product catalog use `deferred` connection; batch backfill / discount-driven jobs use the application default queue.
...
5. Job layer — retries, uniqueness, `ShouldQueueAfterCommit` for catalog upsert/variant-delete, stable idempotency keys from event/job construction, **`dispatch(...)->onConnection(queue_connection)` (default `deferred`) for profiles/events/orders/single-product catalog** / **bare `dispatch()` for batch backfill & discount-driven jobs**.
...
8. Support layer — `CatalogExternalIdStore`, `KlaviyoLogger`; Concerns — `ResolvesDiscountables`.
9. Provider bootstrap — `KlaviyoServiceProvider::registerDeferredQueueConnection()` when `queue.connections.deferred` is null.
...
| `queue_connection` | `KLAVIYO_QUEUE_CONNECTION` | **`deferred`** — profiles, subscribe, events, orders, single-product catalog from product/variant/admin lifecycle. Override to `redis` / `database` / `sync` when ops wants workers. Batch backfill / discount re-syncs ignore this and use bare `dispatch()` |
...
**Queue dispatch rule:**
- Consent/profile/storefront/order and product/variant/admin single-catalog paths: `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` (mirror GTMEventHandler; no helper class). Nested parent sync from `DeleteCatalogVariantFromKlaviyo` also uses deferred.
- `klaviyo:sync-all-products` → `SyncAllProductsToKlaviyo` bulk chunks via `syncProductsBulk`; `ResolvesDiscountables::dispatchProductSyncs` / `dispatchFullCatalogSync` (all discount listeners) — bare `dispatch()` on the application default queue. Do not call `onConnection('deferred')` on these. Do not fan out per-product upsert jobs for backfill.
- `klaviyo:delete-all-products` — dispatches `DeleteAllProductsFromKlaviyo` on application default queue (not deferred).
- `KlaviyoServiceProvider::registerDeferredQueueConnection()` must set `queue.connections.deferred` when null; `registerDeferredQueueMiddleware()` must register `InvokeDeferredCallbacks` on the HTTP kernel when absent.
...
| `SyncProductOnVariantDeleted` | `ProductVariantDeletedEvent` | `enabled` && `sync_products` | `DeleteCatalogVariantFromKlaviyo` by captured variant SKU (`resolveVariantExternalId`); then parent UPDATE or DELETE if no variants remain; also remember item SKU in store |
...
| `SyncProductsOnDiscountUpdated` | `DiscountUpdatedEvent` | `enabled` && `sync_products` | Skip coupon discounts; skip irrelevant fields (`uses`, `updated_at`); limited → product syncs; global → `SyncAllProductsToKlaviyo` (`ResolvesDiscountables`); application default queue |
| `SyncProductsOnDiscountBecameLimited` | admin `BeforeDiscountLimitationAttached` (if present) | `enabled` && `sync_products` | First limitation on former global → `SyncAllProductsToKlaviyo`; application default queue |
| `SyncProductsOnDiscountLimitationChanged` | admin `DiscountLimitationAttached` / `DiscountLimitationDetached` (if present) | `enabled` && `sync_products` | Sync related products (product/variant/brand/collection); application default queue |
| `SyncProductsOnDiscountBecameGlobal` | admin `DiscountLimitationDetached` (if present) | `enabled` && `sync_products` | Last limitation removed → `SyncAllProductsToKlaviyo`; application default queue |
| `SyncProductsOnDiscountDeleted` | admin `DiscountDeleted` (if present) | `enabled` && `sync_products` | Related products/collections or full sync if none; application default queue |

#### Catalog commands (register in `KlaviyoServiceProvider`)

| Command | Behavior |
|---------|----------|
| `klaviyo:sync-all-products {--chunk=100}` | Requires `enabled` && `sync_products`. Dispatches `SyncAllProductsToKlaviyo` which chunks products (eager-load variants/collections/brand/media; chunk size ≤100 to match Klaviyo bulk limits), and for each chunk that passes availability/published checks, submits **bulk catalog item job(s)** then **bulk catalog variant job(s)** via `KlaviyoCatalogService::syncProductsBulk`. Used to backfill the existing catalog so Marketing recommendations have the full set. Application default queue (no deferred). **Forbidden:** spawning one `SyncProductToKlaviyo` per product for backfill. |
| `klaviyo:delete-all-products {--force} {--page-size=100}` | Requires `enabled` only (not `sync_products`). Confirms unless `--force`. Dispatches `DeleteAllProductsFromKlaviyo` on the application default queue; job lists remote catalog items via `GetCatalogItemsRequest`, then submits `BulkDeleteCatalogItemsRequest` via `deleteAllCatalogItems()` (≤100 per request). Variants are removed with parent items. Ops wipe / reset path. |
...
ProductPublished → SyncProductToKlaviyo(CREATE|UPDATE) → KlaviyoCatalogService::syncProduct (synchronous PATCH item + variants when updating; bulk jobs for creates)
ProductUpdated → SyncProductToKlaviyo(UPDATE) → syncProduct (synchronous PATCH when remote exists) or deleteByExternalIds when unavailable
...
ProductVariantCreated|Updated → SyncProductToKlaviyo(UPDATE parent) → syncProduct (synchronous PATCH item + each variant when updating)
ProductVariantDeleted → DeleteCatalogVariantFromKlaviyo → deleteCatalogVariant
    → parent SyncProductToKlaviyo UPDATE|DELETE (syncProduct with synchronous PATCH when updating)
Admin pricing|options|collections|media|urls → SyncProductToKlaviyo(UPDATE parent) → syncProduct (synchronous PATCH when updating)
Discount updated|limitation|became-global|deleted → ResolvesDiscountables
    → SyncProductToKlaviyo bulk batches / SyncAllProductsToKlaviyo bulk chunks (skip coupons)
klaviyo:sync-all-products → SyncAllProductsToKlaviyo → syncProductsBulk per chunk (≤100 products; bulk jobs only)
klaviyo:delete-all-products → DeleteAllProductsFromKlaviyo → deleteAllCatalogItems (bulk-delete jobs)
```

syncProduct / syncProductsBulk orchestration: pin channel/customer-group → availability check per product → ensure categories (single create) → **resolve create/update per item + per variant via Klaviyo GET** (`fetchRemoteCatalogItemState`) → **lifecycle path (`synchronousUpdates: true`):** PATCH existing item (`UpdateCatalogItemRequest`) and each existing variant (`UpdateCatalogVariantRequest`); bulk-create missing resources with item-create polling → **batch path (default):** item/variant bulk create/update jobs only → poll item bulk create jobs to complete before variant bulk create → variant bulk update payloads omit `relationships` → `CatalogExternalIdStore::remember` → `deleteOrphanCatalogVariants` (404 = skip; single delete requests for orphans).
Both `SyncProductToKlaviyo` and `DeleteCatalogVariantFromKlaviyo` implement `ShouldQueueAfterCommit`.
Marketing profile/subscribe/event/order, single-product catalog from product/variant/admin lifecycle, and nested parent sync from `DeleteCatalogVariantFromKlaviyo` use `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))`.
`SyncAllProductsToKlaviyo` uses bare `dispatch()` (application default queue) and submits bulk jobs directly — **must not** fan out per-product Laravel jobs for upsert.
Discount-driven `ResolvesDiscountables` dispatches either batched bulk sync jobs or `SyncAllProductsToKlaviyo` on the application default queue.
`klaviyo:delete-all-products` → `DeleteAllProductsFromKlaviyo` on application default queue (not deferred).
...
| Catalog item | Bulk Create / Bulk Update Catalog Item jobs (`POST /api/catalog-item-bulk-create-jobs`, `POST /api/catalog-item-bulk-update-jobs`) **or** synchronous PATCH (`PATCH /api/catalog-items/{id}` via `UpdateCatalogItemRequest` on lifecycle single-product updates) | Item `external_id` = first non-empty variant **SKU** (fallback: `(string) product.id` if no SKU; `/` replaced with `-`). Required: title, description, url, categories relationship. Bulk **update** only when item exists remotely (GET variants relationship ≠ 404). Bulk **update does not create** missing items. Lifecycle sync PATCH applies immediately when one item is being updated. Delete by **captured** external_id strings — never re-derive SKU after variants are gone |
| Catalog variant | Bulk Create / Bulk Update Catalog Variant jobs (`POST /api/catalog-variant-bulk-create-jobs`, `POST /api/catalog-variant-bulk-update-jobs`) **or** synchronous PATCH (`PATCH /api/catalog-variants/{id}` via `UpdateCatalogVariantRequest` on lifecycle single-product updates — one request per variant) | One variant per Lunar variant; `external_id` = sanitized variant **SKU** (fallback `(string) variant.id` when no SKU). **Create:** includes `relationships.item` → parent `$custom:::$default:::{item_external_id}`. **Update (bulk or PATCH):** attributes + compound `id` only — **no** `relationships` (Klaviyo rejects `item` on update). Route **per variant**: create if absent remotely, update if present. Lifecycle sync must PATCH each existing variant so product name/title changes propagate to variant records in Klaviyo UI. Variant bulk create runs **after** item bulk create job completes when item was newly created. Variant delete uses captured variant SKU via single delete request |
| Catalog sync product | Service orchestration (`syncProduct`, `syncProductsBulk`) | `syncProduct` calls `syncProductsBulk(collect([product]), synchronousUpdates: true)`. Load variants/collections/brand/media. Build storefront URL like Mailchimp (`app.url` + locale slug). A confirmed `isAvailable() === false` or unpublished status → `deleteProductByExternalIds` (404 = success). If availability evaluation throws, fail/retry—**never** treat the exception as unavailable or delete remotely. Queue workers use an explicit catalog channel/customer-group context. Else ensure categories → resolve remote state → **lifecycle:** PATCH existing item/variants + bulk create missing → **batch:** bulk create/update only → poll item create jobs before variant create → `CatalogExternalIdStore::remember` → **`deleteOrphanCatalogVariants`** (404 skip). Failures wrap `FailedKlaviyoSyncException` |
| Catalog bulk backfill | Command + chunk job | Same eligibility as Mailchimp bulk (`isAvailable()`). `SyncAllProductsToKlaviyo` chunks ≤100 products and calls `syncProductsBulk` — **required** bulk item + variant jobs; no per-product single-item HTTP and no per-product Laravel job fan-out for upsert |
| Catalog wipe | Command + queue job | `klaviyo:delete-all-products` dispatches `DeleteAllProductsFromKlaviyo` on application default queue → `deleteAllCatalogItems()` (paginated `GetCatalogItemsRequest` + `BulkDeleteCatalogItemsRequest`); requires `enabled` only; variants removed with parent items; Klaviyo processes bulk jobs remotely |
...
| Klaviyo profile upsert | Job retries; connection `lunar.klaviyo.queue_connection` default **`deferred`** | Create-or-update by email — upsert |
| Klaviyo subscribe (`ExplicitOptIn`) | Job retries; connection default **`deferred`** | Re-subscribe without `historical_import` is acceptable; avoid duplicate ConsentGranted emits so confirmation emails are not spammed |
| Klaviyo subscribe (`CustomerRegistration`) | Job retries; connection default **`deferred`** | Immediate path via single-opt-in `automatic_list_id` (no `historical_import`) only for new/eligible, non-suppressed profiles. Automatic paths must not remove prior opt-outs/suppressions |
| Klaviyo behavioral event | Job retries; connection default **`deferred`** | Always send API `unique_id` = stable `eventId`. If producer passed `uniqueKey`, that becomes `eventId`. If not, UUID (or equivalent) is created **once** in the event/job constructor and serialized. Map `product_id*` / `variant_id*` to SKU before Create Event. **Forbidden:** new id inside `handle()` / per attempt — that duplicates events when attempt 1 succeeds at Klaviyo and the worker dies before job completion. |
| Klaviyo Placed Order | `ShouldBeUnique` `klaviyo-order-sync-{id}` **and** event `unique_id=order.id`; connection default **`deferred`** | Queue uniqueness ≠ enough; API `unique_id` prevents duplicate metrics on worker retry after successful send. Line `ProductID`/`VariantID` must match catalog identity |
| Klaviyo Ordered Product | Same order job; one event per order line; connection default **`deferred`** | Stable API `unique_id=order:{orderId}:line:{lineId}`; case-sensitive `ProductID`/`VariantID` match catalog identity |
| Klaviyo catalog product upsert | Job retries; `ShouldBeUnique` `klaviyo-product-sync-{id}` with **`uniqueFor = 7200`**; **`ShouldQueueAfterCommit`**; connection default **`deferred`** (product/variant/admin lifecycle) **or** application default when spawned by batch/discount path | **Lifecycle:** `syncProduct` submits synchronous PATCH for existing item + each variant (`UpdateCatalogItemRequest`, `UpdateCatalogVariantRequest`); bulk jobs for creates. **Batch:** bulk create/update jobs only. Re-running sync converges to same remote state; orphan remote variants deleted after acceptance. **Forbidden:** batch path using PATCH; lifecycle path relying on async bulk variant update alone for title changes |
| Klaviyo catalog product delete | Job retries; `ShouldBeUnique` `klaviyo-product-sync-{id}-delete` (**separate** from upsert); **`ShouldQueueAfterCommit`**; connection default **`deferred`** (product lifecycle) | Job carries captured external_id strings + productId scalars — **not** dependent on restoring a force-deleted Eloquent model or re-reading deleted variant SKUs. Reads `CatalogExternalIdStore`; forgets after success. Delete treats 404 as success. **Forbidden:** shared unique id with upsert that can discard DELETE |
| Klaviyo catalog variant delete | Job retries; `ShouldBeUnique` `klaviyo-catalog-variant-delete-{variantExternalId}`; **`ShouldQueueAfterCommit`**; connection default **`deferred`**; nested parent UPDATE/DELETE also deferred | `DeleteCatalogVariantFromKlaviyo`; then parent UPDATE or DELETE |
| Klaviyo catalog backfill | `SyncAllProductsToKlaviyo` on **application default queue (no deferred)** | Chunks ≤100 products; each chunk submits bulk item + bulk variant jobs via `syncProductsBulk`. **Forbidden:** fan-out to per-product upsert jobs |
| Klaviyo discount-driven re-sync | `ResolvesDiscountables` → batched bulk sync / `dispatchFullCatalogSync` on **application default queue (no deferred)** | Limited discount → batched bulk upsert jobs; global / became-global|limited → `SyncAllProductsToKlaviyo` bulk chunks; do not use deferred |
| Klaviyo catalog wipe | `DeleteAllProductsFromKlaviyo` on **application default queue** | Command dispatches job → `deleteAllCatalogItems` lists remote items then `BulkDeleteCatalogItemsRequest`; Klaviyo processes bulk jobs remotely |
...
8. **M8 — Klaviyo package (profiles/events/orders)**: Config (`list_id` + `automatic_list_id`, GA `api_revision`, **`queue_connection` default `deferred`**), connector (JSON:API media headers), services (mode → list; **no** live `historical_import`; automatic paths protect prior suppressions), jobs via `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))`, `registerDeferredQueueConnection()` in `KlaviyoServiceProvider`, listeners, tests (default disabled; assert job connection is `deferred` unless overridden). Order events use case-sensitive catalog `ProductID`/`VariantID`; emit per-line `Ordered Product` alongside `Placed Order`. Behavioral events rewrite `product_id` / `variant_id` to SKU before Create Event.
...
11. **M11 — Klaviyo catalog**: Add `sync_products` config; `KlaviyoCatalogService` + Saloon catalog requests (bulk create/update item + variant jobs for upsert; single create for categories; single/bulk delete unchanged); `SyncProductToKlaviyo` / `SyncAllProductsToKlaviyo` / `DeleteCatalogVariantFromKlaviyo` with `CatalogExternalIdStore` + `CaptureCatalogIdentityOnProductDeleting` + separate DELETE uniqueness + `ShouldQueueAfterCommit`; single-product lifecycle/admin jobs on `queue_connection` default `deferred` (submit bulk jobs of size 1); `SyncAllProductsToKlaviyo` bulk chunks on application default queue; listeners on product **and** variant lifecycle events + admin pricing/options/collections/media/urls; orphan variant cleanup after bulk upsert; availability exceptions retry without deletion; Artisan `klaviyo:sync-all-products`; Pest + MockClient coverage; update Klaviyo skill. Run backfill command in ops after enabling `KLAVIYO_SYNC_PRODUCTS`.
...
13. **M13 — Catalog parity completion (implementation sync):** Discount-driven price re-sync via `ResolvesDiscountables` + discount listeners (skip coupons; global/limited transitions) on **application default queue (bare `dispatch()`)**; Artisan `klaviyo:delete-all-products` wipe (later moved to `DeleteAllProductsFromKlaviyo` job — see M15); config `debug` / metric name overrides / default category env / **`queue_connection`**; tests for orphans, after-commit, admin surfaces, discounts, wipe, **deferred via `onConnection` on single-product/marketing jobs and bare `dispatch()` on discount/batch jobs**.
14. **M14 — Catalog bulk upsert (implemented):** Replace single-item catalog create/update HTTP in **batch** upsert orchestration with Klaviyo bulk jobs: `BulkCreateCatalogItemsRequest`, `BulkCreateCatalogVariantsRequest`, `BulkUpdateCatalogItemsRequest`, `BulkUpdateCatalogVariantsRequest`, `GetBulkCreateCatalogItemsJobRequest`. Refactor `KlaviyoCatalogService::syncProduct` / `syncProductsBulk`; per-item and per-variant create vs update from Klaviyo remote GET; poll item bulk create jobs before variant bulk create; variant update payloads omit `relationships`. Refactor `SyncAllProductsToKlaviyo` to chunk ≤100 and call `syncProductsBulk`. Keep deletes/orphan cleanup on existing single/bulk-delete paths. Update Klaviyo skill + Pest MockClient coverage for bulk payloads, routing, and job polling.
15. **M15 — Lifecycle catalog sync reliability (implemented):** Register `InvokeDeferredCallbacks` middleware via `KlaviyoServiceProvider::registerDeferredQueueMiddleware()`; set `SyncProductToKlaviyo::$uniqueFor = 7200`; add `UpdateCatalogVariantRequest` + `updateCatalogVariant()`; `syncProduct` passes `synchronousUpdates: true` so existing item + variants PATCH synchronously (fixes stale variant titles after admin save); keep batch `syncProductsBulk` on bulk jobs only; make host `TelemetryService` best-effort so terminating callbacks do not abort deferred job drain; Pest for deferred middleware registration + lifecycle PATCH variant updates; catalog wipe via `DeleteAllProductsFromKlaviyo` job on default queue.
...
- Synchronous single-item catalog create/update HTTP in **batch** upsert orchestration (forbidden — bulk jobs required; see revision note 28 / M14). Lifecycle single-product updates **require** synchronous PATCH for existing item + variants (M15).
...
- Klaviyo: Saloon MockClient; assert **different** list targets for `ExplicitOptIn` (`list_id`, no `historical_import`) vs `CustomerRegistration` (`automatic_list_id`, no `historical_import`, no invented `consented_at`); assert automatic paths do not Bulk Subscribe known unsubscribed/spam-suppressed/user-suppressed profiles (and fail-closed on profile GET failure); assert request `Accept`/`Content-Type` use `application/vnd.api+json`; `unique_id` equals `eventId` / order id; retry does not change ids; no Mailchimp heal loop; `Placed Order` + per-line `Ordered Product` use case-sensitive `ProductID`/`VariantID` matching catalog identity and stable distinct ids; behavioral `TrackEventToKlaviyo` rewrites `product_id` / `variant_id` (and indexed keys) from Lunar DB ids to SKU; locale upsert from context/user/app; **profiles/events/orders/single-product catalog use connection `deferred` by default** (assert `$job->connection === 'deferred'` / Bus::assertDispatched with connection check, mirroring GTMEventHandlerTest); override via `lunar.klaviyo.queue_connection` still honored
- Klaviyo catalog: listeners no-op when `enabled`/`sync_products` false; `ProductPublished` dispatches `SyncProductToKlaviyo` on deferred connection; unavailable/unpublished sync path deletes by external ids from `CatalogExternalIdStore` / pre-delete capture; availability exceptions retry and assert no DELETE request was sent; DELETE job survives without reloading deleted variants / force-deleted model; DELETE unique key distinct from upsert; `SyncProductToKlaviyo::$uniqueFor = 7200`; `ShouldQueueAfterCommit` on catalog upsert/variant-delete jobs; **provider registers `InvokeDeferredCallbacks` middleware**; variant create/update/delete listeners keep remote variants current (`DeleteCatalogVariantFromKlaviyo`); **lifecycle sync asserts `UpdateCatalogItemRequest` and `UpdateCatalogVariantRequest` sent (not bulk variant update alone) when updating existing resources**; orphan remote variants deleted after upsert (item 404 during orphan list = skip); admin options/collections/pricing/media/urls dispatch parent UPDATE on deferred; discount suite (coupon skip, limited/global, limitation attach/detach, delete) dispatches on **application default queue (not deferred)**; batch upsert payloads submitted via **bulk create/update** Saloon requests (assert ≤100 resources per job, items before variants); **per-variant create vs update** when item exists but variants missing; **item bulk create job polled** before variant bulk create; variant bulk **update** payloads omit `relationships`; `klaviyo:sync-all-products` dispatches `SyncAllProductsToKlaviyo` bulk chunks on **default queue** (no per-product upsert fan-out); `klaviyo:delete-all-products` dispatches `DeleteAllProductsFromKlaviyo` on default queue; 404 on delete is success
- Core telemetry: `TelemetryService` best-effort on terminate — rate-limited (429) or network failures must not throw uncaught exceptions that abort deferred job drain
- Regression: existing Mailchimp service/job tests still pass
- Config: default `api_revision` is not `2024-10-15`; default `queue_connection` is `deferred` 
...
8. Listeners: enablement → capability → map → `dispatch(...)->onConnection(...)` for deferred paths, or bare `dispatch()` for batch/discount. No helper dispatcher class. No Saloon in listeners.
...
16. Catalog item `external_id` = first variant SKU (fallback product id); variant `external_id` = variant SKU (fallback variant id when no SKU). All Klaviyo-facing product/variant identifiers (catalog, behavioral events, order metrics) use SKU — never raw Lunar DB ids when a SKU exists.
...
23. Catalog upsert jobs delete orphan remote variants after syncing current Lunar variants (SKU identity; legacy DB-id keyed orphans removed).
...
28. Profiles, subscribe, events, orders, and single-product catalog from product/variant/admin lifecycle use `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` (base value `deferred`). Pattern matches lunar-frontend GTM — **no** helper dispatcher class. `SyncAllProductsToKlaviyo` submits bulk catalog jobs on the application default queue (bare `dispatch()`). Discount-driven `ResolvesDiscountables` dispatches batched bulk sync or full bulk backfill on the application default queue. Catalog wipe dispatches `DeleteAllProductsFromKlaviyo` on the application default queue. `KlaviyoServiceProvider::registerDeferredQueueConnection()` ensures `queue.connections.deferred` is registered when absent; `registerDeferredQueueMiddleware()` ensures `InvokeDeferredCallbacks` runs so deferred jobs execute after the response.
29. **Batch** catalog upserts submit Klaviyo async bulk jobs only. **Lifecycle single-product** updates use synchronous PATCH (`UpdateCatalogItemRequest`, `UpdateCatalogVariantRequest`) for existing remote item + variants; creates use bulk jobs with item-create polling. Bulk update does not insert missing resources. Create vs update is chosen per item and per variant from Klaviyo remote GET — not `CatalogExternalIdStore` alone. Poll item bulk create jobs before variant bulk create. Variant bulk update omits `relationships`.
30. `CatalogExternalIdStore` captures DELETE identity; upsert routing uses `fetchRemoteCatalogItemState` / `resolveCatalogItemSyncContext`. `SyncProductToKlaviyo::$uniqueFor = 7200` prevents stuck unique locks after interrupted deferred dispatches.
...
16. **Klaviyo catalog:** gated by `enabled` && `sync_products` (wipe command: `enabled` only); unpublished/unavailable products must not remain recommendation-visible — delete remote catalog item by external ids from `CatalogExternalIdStore` / pre-delete capture; variant price/stock/SKU/delete + admin pricing/options/collections/media/urls + discount price changes must update remote catalog; **batch paths** via bulk upsert jobs; **lifecycle/admin single-product paths** via synchronous PATCH for existing item + variants; DELETE jobs must not depend on force-deleted Eloquent restore or post-delete SKU lookup; upsert uniqueness must not discard DELETE; `SyncProductToKlaviyo::$uniqueFor = 7200`; `ShouldQueueAfterCommit` on upsert/variant-delete; orphan remote variants must be cleaned after upsert; availability-check exceptions must retry without deletion; missing product URL may report Soft/Silent without crashing admin save; respect Catalogs API rate limits (≤100 resources per bulk job, max 500 in-progress Klaviyo jobs) via chunking + queue retries/backoff; category external_ids must avoid Klaviyo-stripped special-character collisions; **forbidden:** batch path using synchronous PATCH; lifecycle path using async bulk variant update alone when item title changed but variant titles must match.
...
24. **Deferred vs default queue (split):** Profiles/events/orders/single-product catalog must not block the HTTP response — use `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))`. Package must register `InvokeDeferredCallbacks` middleware — without it, deferred jobs are queued but never executed (symptom: listener logs dispatch, no `Product sync job started`). Batch backfill (`SyncAllProductsToKlaviyo` bulk chunks), discount-driven re-syncs, and catalog wipe must stay on the application default queue (bare `dispatch()`) and must **not** use deferred. Host terminating callbacks (e.g. Lunar telemetry) must not throw uncaught exceptions after the response — that aborts deferred job drain. Putting batch/discount/wipe bulk catalog work on deferred is a specification violation.
25. **Catalog upsert transport:** batch backfill/discount re-sync must use Klaviyo bulk create/update jobs only. Lifecycle single-product/admin sync must PATCH existing item + each variant synchronously (`UpdateCatalogItemRequest`, `UpdateCatalogVariantRequest`); bulk update must not be used for resources absent in Klaviyo — route to bulk create instead (per item and per variant). Poll item bulk create job completion before variant bulk create. Variant bulk update payloads must omit `relationships`.
26. **Unique job locks:** `SyncProductToKlaviyo` must set `uniqueFor` (e.g. 7200s) so interrupted deferred runs cannot block all future syncs for that product indefinitely.
