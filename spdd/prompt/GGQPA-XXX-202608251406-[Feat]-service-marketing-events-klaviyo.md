...
27. **Deferred vs default queue (split):** Profiles, subscribe, storefront events, orders, and **single-product** catalog upsert/delete from product/variant/admin lifecycle dispatch with `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` — process after the HTTP response (mirror lunar-frontend GTM; no helper dispatcher class). Batch backfill (`SyncAllProductsToKlaviyo` bulk chunks), discount-driven catalog re-syncs (`ResolvesDiscountables` → batched bulk sync / `dispatchFullCatalogSync`), stay on the **application default queue** (bare `dispatch()`, no `onConnection`). Catalog wipe (`klaviyo:delete-all-products`) is **not** a Laravel queue job — the Artisan command calls `KlaviyoCatalogService::deleteAllCatalogItems()` which sends Saloon `BulkDeleteCatalogItemsRequest` synchronously; Klaviyo’s remote bulk-delete jobs process asynchronously. Package ensures `queue.connections.deferred` exists when missing via `KlaviyoServiceProvider::registerDeferredQueueConnection()`. Ops may override deferred connection via `KLAVIYO_QUEUE_CONNECTION`.
28. **Catalog bulk upsert (implemented — M14):** All catalog **upsert** traffic (lifecycle single-product, admin-driven parent sync, backfill, discount-driven re-sync) submits Klaviyo **async bulk catalog jobs** — never synchronous single-item Create/Update Catalog Item/Variant HTTP in the sync hot path. **Create:** `POST /api/catalog-item-bulk-create-jobs` and `POST /api/catalog-variant-bulk-create-jobs`. **Update (existing remote resources only):** `POST /api/catalog-item-bulk-update-jobs` and `POST /api/catalog-variant-bulk-update-jobs`. **Bulk update does not insert** missing items or variants. Service chooses create vs update per **item and per variant** using Klaviyo remote state (`GET /catalog-items/{compoundId}/relationships/variants/` — 404 → create item; 200 + empty → update item + create all variants; 200 + ids → update item + create/update each variant by presence). `CatalogExternalIdStore` alone is **not** sufficient for upsert routing (DELETE capture only). After item bulk create jobs are accepted, **poll** `GET /catalog-item-bulk-create-jobs/{job_id}` until `status=complete` (throw on `failed_count > 0`) **before** variant bulk create — parent item must exist. Variant bulk **update** payloads must **omit** `relationships` (Klaviyo rejects `item` relation on update; create only). Each request accepts ≤100 resources; chunk accordingly. Items bulk jobs first, then variants. Single-product lifecycle may submit bulk jobs of size 1. **Forbidden:** per-product `CreateCatalogItemRequest` / `UpdateCatalogItemRequest` / `CreateCatalogVariantRequest` / `UpdateCatalogVariantRequest` in upsert orchestration. Deletes, orphan cleanup, and wipe stay on existing single/bulk-delete paths; orphan list treats item 404 as no-op.
...
- Profiles, subscribe, events, orders, and single-product catalog upsert/delete from product/variant/admin lifecycle must dispatch with `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` (base value `deferred`), matching lunar-frontend GTM — no helper dispatcher class. Batch catalog backfill (`SyncAllProductsToKlaviyo` bulk chunks) and discount-driven re-syncs (`ResolvesDiscountables`) must use the **application default queue** (bare `dispatch()`, no `onConnection`). Catalog wipe runs sync Saloon bulk-delete HTTP from the Artisan command (not a Laravel queued job). All catalog **upserts** use Klaviyo bulk create/update jobs — not synchronous single-item create/update HTTP.
...
class KlaviyoCatalogService {
    +syncProduct(product) array
    +syncProductsBulk(products) array
...
2. Technical implementation:
   ...
   - **Klaviyo queue connection:**
     ...
     - **Application default queue:** `SyncAllProductsToKlaviyo` (Artisan `klaviyo:sync-all-products`) submits bulk catalog job chunks via `syncProductsBulk`; discount-driven paths via `ResolvesDiscountables` (`dispatchProductSyncs` batched bulk / `dispatchFullCatalogSync`). Bare `dispatch()` — do **not** call `onConnection('deferred')` (batch work must not run in-process after the HTTP response).
     - **Catalog wipe:** `klaviyo:delete-all-products` → `deleteAllCatalogItems()` sends `BulkDeleteCatalogItemsRequest` synchronously in the command process; not dispatched on deferred or the Laravel default queue.
     - `KlaviyoServiceProvider::register()` calls `registerDeferredQueueConnection()` so `queue.connections.deferred` exists when the host’s `queue.php` predates the deferred driver. Mailchimp unchanged.
...
3. Business logic:
   ...
   - **Klaviyo catalog (provider-owned, not a shared marketing event):** Marketing uses Klaviyo Catalogs API so email product-recommendation blocks can resolve the full catalog. Parity target is Mailchimp’s outcome (products present remotely), not Mailchimp’s ecommerce PUT shape.
     ...
     - Catalog HTTP stays in Saloon requests under `packages/klaviyo`; service maps Lunar `Product` + variants → bulk job payloads (+ ensure categories via single create). **Upsert requests (required):** `BulkCreateCatalogItemsRequest`, `BulkCreateCatalogVariantsRequest`, `BulkUpdateCatalogItemsRequest`, `BulkUpdateCatalogVariantsRequest`. **Job status (required for item create):** `GetBulkCreateCatalogItemsJobRequest` — poll until `complete` before variant bulk create; throw on `failed_count > 0`. **Delete/list (unchanged):** `GetCatalogItemsRequest`, `GetCatalogItemVariantIdsRequest`, `DeleteCatalogVariantRequest`, `BulkDeleteCatalogItemsRequest`, `DeleteCatalogItemRequest`. **Forbidden in upsert hot path:** `CreateCatalogItemRequest`, `UpdateCatalogItemRequest`, `CreateCatalogVariantRequest`, `UpdateCatalogVariantRequest`.
...
- `klaviyo:delete-all-products` — sync Saloon bulk-delete in the command (not a Laravel job).
- `KlaviyoServiceProvider::registerDeferredQueueConnection()` must set `queue.connections.deferred` when null — same defensive registration as lunar-frontend.
...
| `klaviyo:delete-all-products {--force} {--page-size=100}` | Requires `enabled` only (not `sync_products`). Confirms unless `--force`. Lists remote catalog items via `GetCatalogItemsRequest`, then sends `BulkDeleteCatalogItemsRequest` synchronously from `deleteAllCatalogItems()` (≤100 per request). Variants are removed with parent items. Ops wipe / reset path. **Not** a Laravel queued job. |
...
ProductPublished → SyncProductToKlaviyo(CREATE|UPDATE) → KlaviyoCatalogService::syncProduct (bulk item + bulk variant jobs, size 1)
ProductUpdated → SyncProductToKlaviyo(UPDATE) → syncProduct bulk upsert or deleteByExternalIds when unavailable
...
ProductVariantCreated|Updated → SyncProductToKlaviyo(UPDATE parent) → syncProduct bulk upsert (+ orphan cleanup after Klaviyo accepts jobs)
ProductVariantDeleted → DeleteCatalogVariantFromKlaviyo → deleteCatalogVariant
    → parent SyncProductToKlaviyo UPDATE|DELETE (bulk upsert when updating)
Admin pricing|options|collections|media|urls → SyncProductToKlaviyo(UPDATE parent) → syncProduct bulk upsert
...
syncProduct / syncProductsBulk orchestration: pin channel/customer-group → availability check per product → ensure categories (single create) → **resolve create/update per item + per variant via Klaviyo GET** (`fetchRemoteCatalogItemState`) → submit item bulk create/update jobs → **poll item bulk create jobs to complete** → submit variant bulk create/update jobs (create payloads include `relationships.item`; update payloads omit `relationships`) → `CatalogExternalIdStore::remember` → `deleteOrphanCatalogVariants` (404 = skip; single delete requests for orphans).
...
`klaviyo:delete-all-products` → `deleteAllCatalogItems` (sync Saloon bulk-delete; not a Laravel job).
...
| Catalog item | Bulk Create / Bulk Update Catalog Item jobs (`POST /api/catalog-item-bulk-create-jobs`, `POST /api/catalog-item-bulk-update-jobs`) | Item `external_id` = first non-empty variant **SKU** (fallback: `(string) product.id` if no SKU; `/` replaced with `-`). Required: title, description, url, categories relationship. Bulk **update** only when item exists remotely (GET variants relationship ≠ 404). Bulk **update does not create** missing items. Delete by **captured** external_id strings — never re-derive SKU after variants are gone |
| Catalog variant | Bulk Create / Bulk Update Catalog Variant jobs (`POST /api/catalog-variant-bulk-create-jobs`, `POST /api/catalog-variant-bulk-update-jobs`) | One variant per Lunar variant; `external_id` = sanitized variant **SKU** (fallback `(string) variant.id` when no SKU). **Create:** includes `relationships.item` → parent `$custom:::$default:::{item_external_id}`. **Update:** attributes + compound `id` only — **no** `relationships` (Klaviyo rejects `item` on update). Route **per variant**: create if absent remotely, update if present. Variant bulk create runs **after** item bulk create job completes when item was newly created. Variant delete uses captured variant SKU via single delete request |
| Catalog sync product | Service orchestration (`syncProduct`, `syncProductsBulk`) | Load variants/collections/brand/media. Build storefront URL like Mailchimp (`app.url` + locale slug). A confirmed `isAvailable() === false` or unpublished status → `deleteProductByExternalIds` (404 = success). If availability evaluation throws, fail/retry—**never** treat the exception as unavailable or delete remotely. Queue workers use an explicit catalog channel/customer-group context. Else ensure categories → resolve remote state → item bulk create/update → poll item create jobs → variant bulk create/update → `CatalogExternalIdStore::remember` → **`deleteOrphanCatalogVariants`** (404 skip). Failures wrap `FailedKlaviyoSyncException`. Bulk job HTTP 202 = accepted; item create jobs polled to completion before variant create |
...
| Catalog wipe | Command + sync Saloon bulk-delete | `klaviyo:delete-all-products` → `listAllCatalogItemIds` + in-process `BulkDeleteCatalogItemsRequest`; requires `enabled` only; variants removed with parent items; Klaviyo processes remotely |
...
| Klaviyo catalog product upsert | Job retries; `ShouldBeUnique` `klaviyo-product-sync-{id}`; **`ShouldQueueAfterCommit`**; connection default **`deferred`** (product/variant/admin lifecycle) **or** application default when spawned by batch/discount path | Submits Klaviyo bulk create/update jobs (items then variants); re-running sync converges to same remote state; orphan remote variants deleted after bulk acceptance. **Forbidden:** single-item create/update HTTP per upsert |
...
| Klaviyo catalog wipe | Artisan command + sync Saloon bulk-delete | `deleteAllCatalogItems` lists remote items then `BulkDeleteCatalogItemsRequest` in-process; Klaviyo processes bulk jobs remotely — not a Laravel queue connection choice |
...
- Synchronous single-item catalog create/update HTTP in upsert orchestration (forbidden — bulk jobs required; see revision note 28 / M14)
...
28. Profiles, subscribe, events, orders, and single-product catalog from product/variant/admin lifecycle use `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` (base value `deferred`). Pattern matches lunar-frontend GTM — **no** helper dispatcher class. `SyncAllProductsToKlaviyo` submits bulk catalog jobs on the application default queue (bare `dispatch()`). Discount-driven `ResolvesDiscountables` dispatches batched bulk sync or full bulk backfill on the application default queue. Catalog wipe is sync Saloon HTTP from the Artisan command. `KlaviyoServiceProvider::registerDeferredQueueConnection()` ensures `queue.connections.deferred` is registered when absent.
29. All catalog upserts submit Klaviyo async bulk jobs (`catalog-item-bulk-create-jobs`, `catalog-variant-bulk-create-jobs`, and bulk-update counterparts). Single-item create/update catalog HTTP in upsert orchestration is forbidden. **Bulk update does not insert** missing resources. Create vs update is chosen per item and per variant from Klaviyo remote GET — not `CatalogExternalIdStore` alone. Poll item bulk create jobs before variant bulk create. Variant bulk update omits `relationships`.
30. `CatalogExternalIdStore` captures DELETE identity; upsert routing uses `fetchRemoteCatalogItemState` / `resolveCatalogItemSyncContext`.
...
16. **Klaviyo catalog:** gated by `enabled` && `sync_products` (wipe command: `enabled` only); unpublished/unavailable products must not remain recommendation-visible — delete remote catalog item by external ids from `CatalogExternalIdStore` / pre-delete capture; variant price/stock/SKU/delete + admin pricing/options/collections/media/urls + discount price changes must update remote catalog via **bulk upsert jobs**; DELETE jobs must not depend on force-deleted Eloquent restore or post-delete SKU lookup; upsert uniqueness must not discard DELETE; `ShouldQueueAfterCommit` on upsert/variant-delete; orphan remote variants must be cleaned after bulk upsert; availability-check exceptions must retry without deletion; missing product URL may report Soft/Silent without crashing admin save; respect Catalogs API rate limits (≤100 resources per bulk job, max 500 in-progress Klaviyo jobs) via chunking + queue retries/backoff; category external_ids must avoid Klaviyo-stripped special-character collisions; **forbidden:** synchronous single-item catalog create/update in upsert hot path.
...
24. **Deferred vs default queue (split):** Profiles/events/orders/single-product catalog must not block the HTTP response — use `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))`. Batch backfill (`SyncAllProductsToKlaviyo` bulk chunks) and discount-driven re-syncs must stay on the application default queue (bare `dispatch()`) and must **not** use deferred. Catalog wipe is sync Saloon bulk-delete (not a Laravel queue choice). Missing host `deferred` connection must be registered by the package. Putting batch/discount bulk catalog work on deferred is a specification violation.
25. **Catalog bulk upsert:** all catalog upserts must use Klaviyo bulk create/update jobs; per-product single-item create/update HTTP is forbidden in upsert orchestration. Bulk update must not be used for resources absent in Klaviyo — route to bulk create instead (per item and per variant). Poll item bulk create job completion before variant bulk create. Variant bulk update payloads must omit `relationships`.
