...
- Profiles, subscribe, events, orders, and single-product catalog upsert/delete from product/variant/admin lifecycle must dispatch with `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` (base value `deferred`), matching lunar-frontend GTM — no helper dispatcher class. Batch catalog backfill (`SyncAllProductsToKlaviyo` + nested per-product jobs) and discount-driven re-syncs (`ResolvesDiscountables`) must use the **application default queue** (bare `dispatch()`, no `onConnection`). Catalog wipe runs sync Saloon bulk-delete HTTP from the Artisan command (not a Laravel queued job).
...
2. Technical implementation:
   ...
   - **Klaviyo queue connection:**
     ...
     - **Application default queue:** `SyncAllProductsToKlaviyo` (Artisan `klaviyo:sync-all-products`), every nested `SyncProductToKlaviyo` it spawns, all discount-driven paths via `ResolvesDiscountables` (`dispatchProductSyncs` and `dispatchFullCatalogSync`). Bare `dispatch()` — do **not** call `onConnection('deferred')` (batch work must not run in-process after the HTTP response).
...
- `klaviyo:sync-all-products` → `SyncAllProductsToKlaviyo` and every per-product job it dispatches; `ResolvesDiscountables::dispatchProductSyncs` / `dispatchFullCatalogSync` (all discount listeners) — bare `dispatch()` on the application default queue. Do not call `onConnection('deferred')` on these.
...
`SyncAllProductsToKlaviyo` + nested per-product jobs and discount-driven `ResolvesDiscountables` dispatches use bare `dispatch()` (application default queue).
...
| Klaviyo catalog product upsert | Job retries; `ShouldBeUnique` `klaviyo-product-sync-{id}`; **`ShouldQueueAfterCommit`**; connection default **`deferred`** (product/variant/admin lifecycle) **or** application default when spawned by batch/discount path | Upsert by stable external_id; re-running sync converges to same remote state; orphan remote variants deleted after upsert |
...
| Klaviyo catalog backfill | `SyncAllProductsToKlaviyo` + nested per-product jobs on **application default queue (no deferred)** | Same as Mailchimp bulk: dispatch many UPDATE jobs; uniqueness on per-product job avoids stampede duplicates; must not run batch work on deferred |
| Klaviyo discount-driven re-sync | `ResolvesDiscountables` → `dispatchProductSyncs` / `dispatchFullCatalogSync` on **application default queue (no deferred)** | Limited discount → many product jobs; global / became-global|limited → full catalog job; do not use deferred |
...
28. Profiles, subscribe, events, orders, and single-product catalog from product/variant/admin lifecycle use `dispatch(...)->onConnection(config('lunar.klaviyo.queue_connection', 'deferred'))` (base value `deferred`). Pattern matches lunar-frontend GTM — **no** helper dispatcher class. `SyncAllProductsToKlaviyo` + nested per-product jobs and discount-driven `ResolvesDiscountables` dispatches use bare `dispatch()` (application default queue). Catalog wipe is sync Saloon HTTP from the Artisan command. `KlaviyoServiceProvider::registerDeferredQueueConnection()` ensures `queue.connections.deferred` is registered when absent.
