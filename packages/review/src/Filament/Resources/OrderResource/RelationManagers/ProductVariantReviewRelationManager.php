...
class ProductVariantReviewRelationManager extends BaseRelationManager
{
    /**
    ...
    {
        return __('lunarpanel.review::plugin.relationManagers.product_variant.title');
    }
    ...
    {
        return __('lunarpanel.review::plugin.relationManagers.product_variant.heading');
    }
    ...
    {
        return [
            ReviewResource::getModelNameTableColumn(__('lunarpanel.review::plugin.table.product_variant.name.label')),
    ...
    /**
     * Get the main form components for the resource.
     */
    protected function getMainFormComponents(): array
    {
        return [
            ...
            ReviewResource::getModelTypeFormComponent(ProductVariant::morphName()),
    ...
    {
        return Select::make('reviewable_id')
            ->label(__('lunarpanel.review::plugin.form.model.product_variant'))
            ->options(function (callable $set, callable $get) {
                $order = Order::find($this->getOwnerRecord()->id);

                if ($order) {
                    return ProductVariant::getReviewableProductVariants($order);
                }
            ...
            ->live()
            ->searchable()
            ->afterStateUpdated(function ($record, callable $set, callable $get) {
                if ($record) {
                    return;
                }
                ...
                $set('attribute_data.full_name', $fullNameField);
            });
    }
}
